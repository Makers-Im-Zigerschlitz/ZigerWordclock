...
class Wordclock():
    ...
    def __init__(self, strip, light_sensor=None):
        ...
        if settings.TRACE:
            print("[function]: Wordclock.__init__()")
    ...
    def clear(self):
        if settings.TRACE:
            print("[function]: Wordclock.clear()")
        for i in range(settings.LED_NUMBER):
            self.strip[i] = (0, 0, 0)

    def notify(self, col):
        if settings.TRACE:
            print("[function]: Wordclock.notify()")
    ...
    def error(self, e):
        if "No network" in str(e):
            ...
            word = self.GEN_ERROR
        if settings.TRACE:
            print("[function]: Wordclock.error()")
        self.clear()
        self.strip[self.WIFI] = col
        for i in word:
            self.strip[i] = col
    ...
    def start_animation(self):
        if settings.TRACE:
            print("[function]: Wordclock.start_animation()")
    ...
    def show(self, h, m, s, mode='normal'):
        if settings.TRACE:
            print(f"[function]: Wordclock.show({h},{m},{s},{mode})")
        self.clear()
        # Stunden
        if m >= 25:
            h += 1
