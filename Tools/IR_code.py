...
if not IR_LEARN:
    ...
    while True:
        ...
        if fuzzy_pulse_compare(IR_codes.CODE_1, detected):
            print('Recieved ON')
            pixels.brightness = 0.1
        if fuzzy_pulse_compare(IR_codes.CODE_2, detected):
            print('Recieved OFF')
            pixels.brightness = 0            
        if fuzzy_pulse_compare(IR_codes.CODE_3, detected):
            print('Recieved RED')
            pixels.fill(colours.RED)
        if fuzzy_pulse_compare(IR_codes.CODE_4, detected):
            print('Recieved GREEN')
            pixels.fill(colours.GREEN)
